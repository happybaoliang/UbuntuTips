...
在注册过程中，coordinator URL和gitlab-ci token可以在http://localhost/help/ci/quick_start/README页面找到。gitlab-ci tags要与.gitlab-ci.yml文件中的tag一致。如果注册成功，在项目首页右侧设置->CI/CD Pipelines->Runners activated for this project项目下就可以看到刚才注册的runner了。
...
官网下载jenkens二进制包安装即可,可以在/etc/default/jenkins中修改jenkins的默认端口HTTP_PORT以防止端口冲突。然后运行下面的命令启动jenkins:
